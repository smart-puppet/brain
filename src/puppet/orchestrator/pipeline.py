...
class Orchestrator:
  ...
  def __init__(
    ...
    *,
    ...
    if delay_ms is None:
      device_frames = int(audio_cfg.get("output_frames_per_buffer", 512))
      write_frames = int(audio_cfg.get("output_write_chunk_frames", 256))
    ...
    self._audio_devices_prepared = False
      ...
      try:
        ...
        self._play = PlaySupervisor(
          ...
          config=PlayConfig(
            ...
            search_forward_ticks=int(follow.get("search_forward_ticks", 12)),
            search_forward_dur_ms=int(follow.get("search_forward_dur_ms", 1500)),
            lost_ticks_max=int(follow.get("lost_ticks", 2)),
            found_m=float(follow.get("found_m", 2.0)),
            seek_giveup_ticks=int(follow.get("seek_giveup_ticks", 240)),
            seek_giveup_s=float(follow.get("seek_giveup_s", 180)),
            seek_map=bool(follow.get("seek_map", True)),
            forward_m_per_s=float(follow.get("forward_m_per_s", 0.30)),
            map_res_m=float(follow.get("map_res_m", 0.1)),
            map_size_m=float(follow.get("map_size_m", 12.0)),
            ...
            seek_face_deg=int(follow.get("seek_face_deg", 90)),
  ...
  def _prepare_audio_devices(self) -> None:
    if self._audio_devices_prepared:
      return
    audio_cfg = self.config.get("audio", {})
    device_index = audio_cfg.get("input_device")
    maybe_reset_respeaker_on_start(self.config, device_index=device_index)
    # Pulse opens the ReSpeaker speaker with a 1s ALSA period unless we reload it.
    configure_low_latency_alsa_output(audio_cfg)
    self._audio_devices_prepared = True
  ...
  def _play_tts_chunk(self, phrase: str, chunk: TtsChunk) -> None:
    ...
    if first_chunk:
      ...
      if self.state != PipelineState.SPEAKING:
        self._set_state(PipelineState.SPEAKING)
      output_ms = 0.0
      latency_fn = getattr(self._playback, "device_latency_ms", None)
      if callable(latency_fn):
        output_ms = float(latency_fn())
      self._latency.mark_speaker(lead_in_ms=lead_in_ms, output_latency_ms=output_ms)
