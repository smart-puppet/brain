...
# espeak / Piper IPA vowels — used for phoneme-granularity jaw flaps.
_VOWEL_PHONEMES = frozenset(
  {
    "a",
    "e",
    "i",
...
def resolve_mouth_granularity(config: dict) -> MouthGranularity:
  mouth_cfg = config.get("puppet", {}).get("mouth", {})
  configured = str(mouth_cfg.get("granularity", "word")).lower()
  if configured not in ("word", "phoneme"):
    raise ValueError(
      f"puppet.mouth.granularity must be 'word' or 'phoneme', got {configured!r}"
    )
...
def phoneme_opens_mouth(phoneme: str, *, vowels_only: bool = False) -> bool:
  """Whether this phoneme should open the binary jaw.

  Default: any non-silence speech phoneme (used for word spans).
  ``vowels_only=True``: open on vowels only so phoneme granularity flaps
  closed on consonants instead of holding open for whole words.
  """
  if phoneme in _SILENCE_PHONEMES:
    return False
  if not vowels_only:
    return True
  base = phoneme.rstrip("ː:")
  return base in _VOWEL_PHONEMES or phoneme in _VOWEL_PHONEMES
...
def phoneme_alignments_to_timeline(
  ...
  *,
  vowels_only: bool = True,
) -> list[MouthEvent]:
  events: list[MouthEvent] = []
  offset = 0
  last_open: bool | None = None
  for align in alignments:
    open_mouth = phoneme_opens_mouth(align.phoneme, vowels_only=vowels_only)
...
def word_cues_from_alignments(
  ...
  *,
  ...
  Words shorter than ``min_open_ms`` are dropped. When the silence between
  two words is shorter than ``min_gap_ms``, they are merged into one open
  window (jaw stays open) instead of skipping the later word.
  """
  if sample_rate <= 0:
    return []
...
def holds_ms_to_timeline(holds_ms: Sequence[int], sample_rate: int) -> list[MouthEvent]:
  """Alternating open/close events from hold durations (fallback flap mode)."""
  if sample_rate <= 0 or not holds_ms:
    return []
  events: list[MouthEvent] = []
  offset = 0
  open_mouth = True
  for ms in holds_ms:
    if int(ms) <= 0:
      continue
    events.append(MouthEvent(sample_offset=offset, open=open_mouth))
    offset += _ms_to_samples(int(ms), sample_rate)
