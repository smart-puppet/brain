...
def speed_to_length_scale(speed: float) -> float | None:
  """Map user-facing speed to Piper ``length_scale`` (<1 faster, >1 slower)."""
  if speed <= 0:
    raise ValueError(f"tts.speed must be > 0, got {speed}")
  if speed == 1.0:
    return None
  return 1.0 / speed


def _build_syn_config(*, speed: float, speaker_id: int | None) -> SynthesisConfig:
...
class PiperTts(TtsBackend):
  ...
  def __init__(
    ...
    *,
    ...
    mouth_mode: str = "word",
    mouth_granularity: MouthGranularity = "word",
  ) -> None:
    self._speaker_id = speaker_id
    self._speed = float(speed)
    self._syn_config = _build_syn_config(speed=self._speed, speaker_id=speaker_id)
    self._stopped = False
    self._n_threads = max(0, int(n_threads))
    self._model_path = model_path
    self._mouth_timing = mouth_timing or MouthTiming()
    self._mouth_mode = mouth_mode
    self._mouth_granularity = mouth_granularity
    ...
    if self._include_alignments and self._mouth_mode == "word":
      align_note = f"{self._mouth_granularity} mouth timeline"
    speed_note = f"speed={self._speed:g}x" if self._speed != 1.0 else "speed=1x"
    logger.info(
      "Loaded Piper voice: %s (%d Hz, %s, n_threads=%s, %s)",
      model_path,
      self._sample_rate,
      speed_note,
      thread_note,
  ...
  def synthesize_stream(self, text: str) -> Iterator[TtsChunk]:
    ...
    use_stream_raw = (
      self._speed == 1.0
      and not self._include_alignments
      and hasattr(self._voice, "synthesize_stream_raw")
    )
    if use_stream_raw:
    ...
    for chunk in self._voice.synthesize(
      ...
      if self._include_alignments and self._mouth_mode == "word":
        timeline = timeline_for_audio_chunk(
          ...
          granularity=self._mouth_granularity,
        )
        if timeline:
          logger.debug(
            "TTS mouth timeline (%d %s events, chunk %d smp) for %r",
            len(timeline),
            self._mouth_granularity,
            int(samples.size),
            text[:60],
          )
        if self._mouth_granularity == "word":
          ...
          if word_cues:
            logger.debug(
              "TTS word cues (%d in this audio chunk) for %r",
              len(word_cues),
              text[:60],
            )
...
def create_tts(config: dict[str, Any]) -> TtsBackend:
  ...
  mouth_granularity = resolve_mouth_granularity(config) if mouth_enabled else "word"
  tts = PiperTts(
    model_path=model_path,
    ...
    mouth_granularity=mouth_granularity,
  )
