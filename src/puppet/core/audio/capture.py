...
class AudioPlayback:
  ...
  def drain(self, *, timeout: float | None = None) -> bool:
    """Flush remaining PCM and wait until the estimated playhead catches up.

    ``flush`` only pushes bytes into the ALSA buffer; without a drain the
    process can exit (or ``stop_stream``) while the last word is still queued.
    """
    self.flush()
    ...
    # Playhead is write-clock based; give the hardware buffer time to empty.
    time.sleep(device_s + 0.05)
    return ok
