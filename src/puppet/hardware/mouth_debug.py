...
class MouthDebug:
  ...
  def timeline_coalesced(self, skipped: int, *, applied: object) -> None:
    state = "OPEN" if getattr(applied, "open_mouth", False) else "CLOSED"
    self._log(
      "coalesced %d due events → %s @ %s (gen=%d)",
      skipped,
      state,
      _ms(int(getattr(applied, "target_samples", 0)), self._sample_rate),
      int(getattr(applied, "generation", 0)),
    )
