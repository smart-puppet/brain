...
def test_phoneme_opens_mouth() -> None:
  ...
  assert phoneme_opens_mouth("h")
  assert phoneme_opens_mouth("a", vowels_only=True)
  assert not phoneme_opens_mouth("h", vowels_only=True)
  assert phoneme_opens_mouth("ə", vowels_only=True)
  assert phoneme_opens_mouth("iː", vowels_only=True)
...
def test_word_alignments_merges_when_next_open_too_soon_after_close() -> None:
  sr = 1000
  alignments = [
    _Align("^", 10),
    _Align("a", 120),
  ...
  # Gap between words is 30ms < min_gap 100ms → keep jaw open across both.
  events = word_alignments_to_timeline(
    alignments, sample_rate=sr, min_open_ms=100, min_gap_ms=100
  )
  assert events == [
    MouthEvent(10, True),
    MouthEvent(540, False),
  ]


def test_word_cues_merge_short_gaps() -> None:
  sr = 1000
  alignments = [
    _Align("^", 10),
    _Align("a", 120),
  ...
  cues = word_cues_from_alignments(
    alignments, sample_rate=sr, min_open_ms=50, min_gap_ms=50
  )
  assert cues == [WordCue(10, 300), WordCue(380, 530)]


def test_resolve_mouth_granularity() -> None:
  assert resolve_mouth_granularity({"puppet": {"mouth": {"granularity": "phoneme"}}}) == "phoneme"
  assert resolve_mouth_granularity({"puppet": {"mouth": {}}}) == "word"
  with pytest.raises(ValueError, match="granularity must be"):
    resolve_mouth_granularity({"puppet": {"mouth": {"granularity": "syllable"}}})


def test_timeline_for_audio_chunk_phoneme_granularity() -> None:
  ...
  events = timeline_for_audio_chunk(
    ...
    audio_samples=210,
    sample_rate=1000,
    timing=MouthTiming(servo_move_ms=0, min_close_ms=0, min_open_ms=0),
    granularity="phoneme",
  )
  assert len(events) >= 2
  assert any(e.open for e in events)
  assert any(not e.open for e in events)
...
def test_holds_ms_to_timeline_alternates_open_close() -> None:
  events = holds_ms_to_timeline([100, 100], sample_rate=22050)
  assert events[0].open is True
  assert events[1].open is False
  assert events[0].sample_offset == 0
  assert events[1].sample_offset > events[0].sample_offset
...
def test_phoneme_alignments_to_timeline() -> None:
  ...
  # Vowels-only (phoneme granularity): open on "a", closed on consonants.
  events = phoneme_alignments_to_timeline(alignments)
  assert events == [
    MouthEvent(300, True),
    MouthEvent(600, False),
  ]
  # Speech-vs-silence (legacy): any non-silence phoneme opens.
  events_all = phoneme_alignments_to_timeline(alignments, vowels_only=False)
  assert events_all == [
    MouthEvent(100, True),
    MouthEvent(600, False),
    MouthEvent(650, True),
