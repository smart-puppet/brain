...
def _base_cfg(**overrides: object) -> dict:
  cfg = {
    ...
    "aec": {"enabled": False, "reference_sample_rate": 22050},
...
def test_prefill_scheduled_on_partial_when_enabled() -> None:
  cfg = _base_cfg()
  cfg["llm"] = {
    "backend": "llama_server",
    "model_path": "x.gguf",
    "prefill_during_listen": True,
    "prefill_debounce_ms": 0,
    "prefill_min_chars": 3,
  }
  fake_llm = FakeLlm()
  orch = _with_fake_playback(
    Orchestrator(cfg, stt=FakeStt(), llm=fake_llm, tts=FakeTts(), vad=PassthroughVad())
  )
  orch._set_state(PipelineState.LISTENING)
  orch._on_stt_partial("hello")
  assert fake_llm.prefill_calls == 1
  orch._last_prefill_at = time.monotonic() - 1.0
  orch._on_stt_partial(" there")
  assert fake_llm.prefill_calls == 2
...
def test_barge_in_cancel_on_sustained_clean_speech() -> None:
  ...
  assert orch._tick_barge_in_cancel(loud, loud)
  assert orch.state == PipelineState.LISTENING
  assert fake_playback.stop_calls >= 1
...
def test_barge_in_grace_covers_inter_phrase_gap() -> None:
  ...
  assert not orch._tick_barge_in_cancel(loud, loud)
...
def test_phrase_end_keeps_aec_during_reply() -> None:
  cfg = _base_cfg()
...
def test_generation_done_finalizes_stt_and_clears_aec(orchestrator: Orchestrator) -> None:
  orchestrator.conversation.draft_user = "Hi"
  orchestrator._audio_ref.write(np.ones(100, dtype=np.float32))
  ...
  assert orchestrator._audio_ref.recent_rms() == 0.0
